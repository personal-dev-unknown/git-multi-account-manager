// crates/gm_adapters/src/persistence/mysql/git_operation_repository.rs
//
// Records discrete git operations (clone, pull, push, etc.) for the operation
// audit log. This is distinct from sync_sessions which track higher-level
// push/pull cycles. A single sync session may produce multiple git_operations
// entries (stage, commit, push).
...
#[derive(Debug)]
pub struct MySqlGitOperationRepository {
    pool: MySqlPool,
}

impl MySqlGitOperationRepository {
    pub fn new(pool: MySqlPool) -> Self { Self { pool } }
}

#[async_trait]
impl GitOpRepository for MySqlGitOperationRepository {
